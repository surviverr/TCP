...
############################################ 日志数据处理 #########################################################
rwnd = 44000 # 接收窗口长度（byte） 【自行调整：填写自己完整接收窗口的大小】
    ...
    if len_value>0:
        ...
        if (len(last_recv)>0):
            for tmp_ind in range(lastpkt_ind+1, len(l_base_rwnd.keys())):
                ...
                # lastpkt_t < tkey3
                if len(last_recv):
                    index_base = np.searchsorted(np.array(last_recv)[:,0], l_base_rwnd[l_base_rwnd_tkey3], side='right')
        ...
        if len(list(l_base_rwnd.keys())) > l_base_rwnd_ind:
            l_base_rwnd_tkey2 = list(l_base_rwnd.keys())[l_base_rwnd_ind]          # 找到 l_base_rwnd 中当前接收事件 后的 最近的事件的时间
        else:
            l_base_rwnd_tkey2 = list(l_base_rwnd.keys())[-1]
...
############################################ 视频制作 #########################################################
time_extend = 30 # 减速倍数 1:原速；2:0.5倍速 【自行调整】
...
def make_frame(t):
    ...
    tkey = time_keylist_base_swnd[1]
    for i in range(1, len(time_keylist_base_swnd)):
        if t < (time_keylist_base_swnd[i] - tkey) * time_extend:
            tkey = time_keylist_base_swnd[i-1]
            break
    
    tkey_nxseq = time_keylist_next_seq[1]
    for i in range(1, len(time_keylist_next_seq)):
        if t < (time_keylist_next_seq[i] - tkey_nxseq) * time_extend:
            tkey_nxseq = time_keylist_next_seq[i-1]
            break
            
    tkey_swnd = time_keylist_swnd[1]
    for i in range(1, len(time_keylist_swnd)):
        if t < (time_keylist_swnd[i] - tkey_swnd) * time_extend:
            tkey_swnd = time_keylist_swnd[i-1]
            break
    
    tkey_base_rwnd = time_keylist_base_rwnd[1]
    for i in range(1, len(time_keylist_base_rwnd)):
        if t < (time_keylist_base_rwnd[i] - tkey_base_rwnd) * time_extend:
            tkey_base_rwnd = time_keylist_base_rwnd[i-1]
            break
    
    tkey_recv_list = time_keylist_recv_list[1]
    for i in range(1, len(time_keylist_recv_list)):
        if t < (time_keylist_recv_list[i] - tkey_recv_list) * time_extend:
    ...
    # part_base_loc <= part_end_loc <= part_nextseq_loc
    elif part_base_loc <= part_end_loc and part_end_loc <= part_nextseq_loc:
        # send noack
        part_send_noack = pch.Rectangle(xy=(part_base_loc, 2), width=part_nextseq_loc-part_base_loc, height=0.75, color='red', linewidth=3)
        ax.add_patch(part_send_noack)

    # part_end_loc <= part_nextseq_loc <= part_base_loc
    elif part_end_loc <= part_nextseq_loc and part_nextseq_loc <= part_base_loc:
        # send no ack tail
        part_send_noack = pch.Rectangle(xy=(part_base_loc, 2), width=len_50m-part_base_loc, height=0.75, color='red', linewidth=3)
        ax.add_patch(part_send_noack)
        # send no ack head
    ...
    # part_nextseq_loc <= part_base_loc <= part_end_loc
    elif part_nextseq_loc <= part_end_loc and part_end_loc <= part_base_loc: 
        # send no ack tail
        part_send_noack = pch.Rectangle(xy=(part_base_loc, 2), width=len_50m-part_base_loc, height=0.75, color='red', linewidth=3)
        ax.add_patch(part_send_noack)
